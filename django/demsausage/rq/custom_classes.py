from demsausage.rq.rq_utils import (
    does_job_want_to_be_unique,
...
class CustomQueue(Queue):
    ...
    def enqueue_call(self, *args, **kwargs):
        if (
            kwargs["meta"] is not None
            and "_custom_job_name" in kwargs["meta"]
            and kwargs["meta"]["_custom_job_name"] is not None
        ):
            custom_job_name = kwargs["meta"]["_custom_job_name"].format(
                **kwargs["kwargs"]
            )
        ...
        if does_job_want_to_be_unique(kwargs["meta"]) is True:
            ...
            if is_job_started_or_queued(custom_job_name) is True:
                logger.info(
                    f"Skip enqueueing job '{custom_job_name}', another is already queued or started"
                )
                return None

        elif does_job_want_to_be_unique_scheduled_job(kwargs["meta"]) is True:
            logger.info(
                f"Ensuring task {custom_job_name} is unique amongst scheduled jobs"
            )
            if is_job_queued(custom_job_name) is True:
                logger.info(
                    f"Skip enqueueing job '{custom_job_name}', another is already queued"
                )
                return None
    ...
    def create_job(self, *args, **kwargs):
        kwargs["on_success"] = Callback(report_job_success)
        kwargs["on_failure"] = Callback(report_job_failure)
...
class CustomWorker(Worker):
    ...
    def perform_job(self, *args, **kwargs):
        ...
        logger.debug(
            f"job {job.func_name} ({job.id}) does_job_want_to_be_unique = {does_job_want_to_be_unique_check}"
        )

        duplicate_jobs = get_duplicate_jobs(job)
        logger.debug(
            f"job {job.func_name} ({job.id}) get_duplicate_jobs for {job.func_name} = {duplicate_jobs}"
        )

        if does_job_want_to_be_unique_check is True and len(duplicate_jobs) > 0:
            logger.warning(
                f"Avoiding performing job {job.func_name} ({job.id}) in queue {job.origin} because it is already queued or started somewhere else"
            )
            return
