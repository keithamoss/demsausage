...
export class NomsReader {
  ...
  public getFoodIconNamesFromNoms() {
    ...
    return Object.keys(this.noms).filter((nomsName: string) =>
      NomsReader.foodNomsIcons.includes(nomsName)
    );
  }
  ...
  public getIconForNoms(spriteIcons: any) {
    ...
    if (this.onlyHasExtraNoms()) {
      ...
      return firstExtraNomsIcon in spriteIcons
        ? spriteIcons[firstExtraNomsIcon]
        : spriteIcons.unknown;
    }
  ...
  public getDetailedIconsForNoms(
    ...
    primaryIcon
      .getImage()
      .setAnchor([iconSize * scaleFactor, iconSize * scaleFactor]);
      ...
      new Style({
        text: new Text({
          ...
          offsetY:
            replaceCount === 0
              ? -(fontSize * 1.15)
              : -(fontSize * numberOfLines) + fontSize * 0.25,
          fill: new Fill({
            color: "#000",
          }),
          stroke: new Stroke({
    ...
    spriteIconNames.forEach((iconName: any, index: number) => {
      ...
      icon
        .getImage()
        .setAnchor(this.getIconAnchorPosition(index, iconSize, scaleFactor));
