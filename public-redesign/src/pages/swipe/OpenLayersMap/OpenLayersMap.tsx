...
class OpenLayersMap extends React.PureComponent<IProps, {}> {
  ...
  componentDidMount() {
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    const polygon = new Polygon(election.geom.coordinates).transform(
      "EPSG:4326",
      "EPSG:3857"
    );
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  ...
  componentDidUpdate(prevProps: IProps) {
    if (this.map !== null) {
      if (prevProps.mapSearchResults !== this.props.mapSearchResults) {
        ...
        JSON.stringify(prevProps.mapFilterOptions) !==
        JSON.stringify(this.props.mapFilterOptions)
      ) {
        // The user has changed the noms filter options
        const sausageLayer = this.getLayerByProperties(
          this.map,
          "isSausageLayer",
          true
        );
        if (sausageLayer !== null) {
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
          sausageLayer.setStyle(
            (feature: IMapPollingPlaceFeature, resolution: number) =>
  ...
  componentWillUnmount() {
    ...
    this.timeoutIds.forEach((timeoutId: number) =>
  ...
  private onVectorSourceChanged(event: BaseEvent) {
    const { /*geojson, */ mapSearchResults, onMapDataLoaded, onMapLoaded } =
      this.props;
  ...
  private getSearchResultsVectorLayer(_map: Map) {
    ...
    if (mapSearchResults !== null) {
      const iconFeature = new Feature({
        geometry: new Point(
          transform(
            [mapSearchResults.lon, mapSearchResults.lat],
            "EPSG:4326",
            "EPSG:3857"
          )
        ),
  ...
  private getMapDataVectorLayer(_map: Map) {
    const { election, /*geojson, */ mapFilterOptions, onMapBeginLoading } =
      this.props;
    ...
    const vectorSource = new VectorSource({
      ...
      async loader(
        ...
        if (geojson === undefined) {
          ...
          onMapBeginLoading();
          const url = `${getAPIBaseURL()}/0.1/map/?election_id=${
            election.id
          }&s=${Date.now()}`;
    ...
    this.vectorSourceChangedEventKey = vectorSource.once(
      "change",
      this.onVectorSourceChanged.bind(this)
    );
  ...
  private zoomMapToSearchResults(map: Map) {
    ...
    if (mapSearchResults !== null) {
      ...
      if (mapSearchResults.extent !== null) {
        ...
        view.fit(
          transformExtent(mapSearchResults.extent, "EPSG:4326", "EPSG:3857"),
          {
            size: map.getSize(),
            ...
            padding:
              mapSearchResults.padding !== undefined
                ? [0, 0, 50, 0]
                : [85, 0, 20, 0],
            callback: (completed: boolean) => {
              if (completed === true) {
        ...
        mapSearchResults.lat !== null &&
        mapSearchResults.lon !== null
      ) {
        view.fit(
          new Point(
            transform(
  ...
  private clearSearchResultsVectorLayer(map: Map) {
    const searchResultsVectorLayer = this.getLayerByProperties(
      map,
      "isSearchIndicatorLayer",
      true
    );
