...
define("MAPPIFY_GEOCODER_API_URL", "https://maps.googleapis.com/maps/api/geocode/json");
...
function googleGeocoder($geocoderParams) {
  $ch = curl_init();

  if(isset($geocoderParams["premises"]) === true && isset($geocoderParams["address"]) === true) {
    $geocoderParams["address"] = $geocoderParams["premises"] . ", " . $geocoderParams["address"];
    unset($geocoderParams["premises"]);
  }

  curl_setopt($ch, CURLOPT_URL, MAPPIFY_GEOCODER_API_URL . "?" . http_build_query(["key" => GOOGLE_API_KEY, "address" => $geocoderParams["address"]]));
...
function addGeocoderResult($params) {
  ...
  $insert = fieldsToInsertSQL("geocoder_results_google", $geocoderAllowedFields, array_keys($params), $params);
...
function getGeocoderResultFromDB($geocoderParams) {
  ...
  $sql = "SELECT * FROM geocoder_results_google WHERE address = :address AND state = :state";
